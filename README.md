...
* ***iframe_image_parameters*** - параметры рамки
   ...
   * ***fit_threshold*** (число, по умолчанию 0.03) - пороговое отклонение масштабированного изображения от параметров рамки после которого оно дополняется черными полосами
...
#### Работа с промптами
   Просмотр промптов. Добавление, удаление, редактирование промптов. В том числе промптов с плейсхолдерами и списка глобальных плейсхолдеров.

Постановка промпта в очередь на генерацию. При постанвкет промпта в очередь, следующий запрос изображения будет выполнен с выбранным промптом в выбранном провайдере.
### Метрики
Метриками покрыты:
...
В нём находится каталог ```original```. 
В котором хрантся изображения в оригинальном размере.
